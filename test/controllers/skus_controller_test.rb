...
class SkusControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should show sku prices in index" do
    @sku.update(current_price_usd: 99.99, original_price_usd: 129.99)
    get skus_url
    assert_response :success
    assert_select "span.text-xl.font-black.text-blue-600", text: /\$99\.99/
    assert_select "span.text-slate-400.line-through", text: /\$129\.99/
  end
  ...
  test "should show sku and show category banner" do
    ...
    assert_response :success
    assert_select '[data-controller="category-tabs"]'
    assert_select "button", text: /Add to Cart/
    assert_select 'div[data-controller="share"]'
  end
end
